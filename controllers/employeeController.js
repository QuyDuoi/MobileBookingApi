...
exports.add_employee = async (req, res, next) => {
    try {
        const { fullName, email, password, phoneNumber, address, id_store } = req.body;
        let image = '';
        ...
        const employee = new Employee({ fullName, email, password, phoneNumber, address, image, id_store });
...
exports.update_employee = async (req, res, next) => {
    try {
        const { id } = req.params;
        const { fullName, email, phoneNumber, address, id_store } = req.body;
        ...
        if (req.file) {
            employee.image = `${req.protocol}://${req.get('host')}/public/uploads/${req.file.filename}`;
        }
